...
|用Excel批量导入学生信息|POST:php/add_stu_excel.php 上传参数:file:xxx(bootstrap file-input实现),extradata={class_id:xxx}| [{"id":"xxx","name":"xxx","department":"xxx","major":"xxx"}...]|是
...
|发布文章|type:POST,url:add_article.php,data:lesson_id=xxx&id=xxx&title=xxx&content=xxx|发布成功：{"if_success":1,error_message:"null"};发布失败：{"if_success":0,"error_messgae":"xxx"}|成功或失败返回的是整数1或0,时间由后端获取|是
|修改文章|type:'POST',data:{article_id:article_id,title:title,content:content},|修改成功：{"if_success":1,error_message:"null"};修改失败：{"if_success":0,"error_messgae":"xxx"}|成功或失败返回的是整数1或0,时间由后端获取|是
