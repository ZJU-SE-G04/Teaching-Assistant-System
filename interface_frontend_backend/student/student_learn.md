...
|发表二级回复                |posts_handler.php                   |action=submitReRe&topic_id=xxx&floor=xxx&id_be_re=xxx&name_be_re=xxx&content=xxx | { "state": 0, "msg":"xxx" } |state为0表示发表成功，为1表示发表失败. post的参数中id_be_re是被回复人的id，如果是0表示被回复的对象是一级回复,如果为1表示被回复的对象是二级回复| 
###选择测试或实验报告
<table>
    <thead>
    <tr>
